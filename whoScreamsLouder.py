...
Example of  "who screams louder" tournaments between three players.
    ...
    If k=50 and the list is [ "A ƐÈÜ", "BEAR" , "c Ʈ ´ ."]
        ...
        In the end 0 has 0 challenges, 1 has 2 challenge and 2 has 1
...
def ex(matches, k):
    ...
    # removing spaces and tabs and then appending them to match[]
    for i in matches:
        j = str(i).replace(' ', '').replace('   ', '').replace('\t', '')
        match.append(j)

    # appending 0 to the length of the elements in match[]
    for _ in match:
        pointlist.append(0)

    # iterating from each element in match[] with no repeat and no self-comparison
    for i in range(len(match)):
        for j in range(i + 1, len(match)):
            ...
            for c in range(len(string1)):
                diff = ord(string1[c]) - ord(string2[c])
                absdiff = -1 * diff
                if diff > 0:
                    if k >= diff:
                        pointa += 1
            ...
            if pointa > pointb:
                ...
                if sum(ord(char) for char in string1) < sum(ord(char) for char in string2):
                    pointlist[i] += 1
                elif sum(ord(char) for char in string1) > sum(ord(char) for char in string2):
                    pointlist[j] += 1
                else:
                    if string1 < string2:
                        pointlist[i] += 1
    ...
    # making an index dic which is indexing elements by their points
    indexdic = dict(zip(indexes, pointlist))

    # from highest to lowes points
